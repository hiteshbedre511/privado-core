...
package object language {
  ...
  implicit class StepsForProperty(val trav: Iterator[JavaProperty]) extends AnyVal {
    ...
    def usedAt: Iterator[CfgNode] = trav.out(EdgeTypes.IS_USED_AT).cast[CfgNode]
  ...
  implicit class NodeTravToProperty(val trav: Iterator[AstNode]) {
    def originalProperty: Iterator[JavaProperty] = trav.out(EdgeTypes.ORIGINAL_PROPERTY).cast[JavaProperty]
  }
