...
object FutureUtil {
  ...
  /** Returns the result of the provided future within the given time or a timeout exception, whichever is first This
    * uses Java Timer which runs a single thread to handle all futureWithTimeouts and does not block like a Thread.sleep
    * would
    * @param future
    *   Caller passes a future to execute
    * @param timeout
    *   Time before we return a Timeout exception instead of future's outcome
    * @return
    *   Future[T]
    */
