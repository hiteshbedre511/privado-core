        ...
        .flatMap(methodParameter => {
          ExporterUtility.convertIndividualPathElement(methodParameter) match {
            case Some(pathElement) =>
              getCollectionOccurrenceModel(methodParameter.iterator.method, pathElement)
      ...
      methodLocalOccurrences
        .flatMap(localVar => {
          ExporterUtility.convertIndividualPathElement(localVar) match {
            case Some(pathElement) => getCollectionOccurrenceModel(localVar.iterator.method, pathElement)
            case None              => None
          }
        })
    )
      ...
      methodLiteralOccurrences
        .flatMap(literal => {
          ExporterUtility.convertIndividualPathElement(literal) match {
            case Some(pathElement) => getCollectionOccurrenceModel(literal.iterator.method, pathElement)
            case None              => None
          }
        })
    )
